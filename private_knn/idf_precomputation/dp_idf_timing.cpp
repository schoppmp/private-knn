...
extern "C" {
  #include "private_knn/idf_precomputation/dp_idf.h"
}
...
protected:
  void validate() {
    ...
    if(num_samples.size() == 0) {
      BOOST_THROW_EXCEPTION(po::error("'num_samples' must be passed at least once"));
    }
    ...
    if(vocabulary_size < num_samples) {
      BOOST_THROW_EXCEPTION(po::error("'vocabulary_size' must be at least 'num_samples'"));
    }
    if(epsilon <= 0) {
...
public:
  ...
  dp_idf_config() {
    namespace po = boost::program_options;
    add_options()
      ("vocabulary_size,V", po::value(&vocabulary_size)->composing(),
        "Number of words in the vocabulary")
      ("num_samples,L", po::value(&num_samples)->composing(), "Number words to sample")
...
int main(int argc, const char *argv[]) {
  ...
  size_t num_experiments = std::max({conf.vocabulary_size.size(), conf.num_samples.size()});
  ...
  for(size_t num_runs = 0; conf.max_runs < 0 || num_runs < conf.max_runs; num_runs++) {
    ...
    for(size_t experiment = 0; experiment < num_experiments; experiment++) {
      ...
      for(size_t i = 0; i < vocabulary_size; i++) {
        probabilities[i] = std::exp(count_max/(i+1)* epsilon / (2 * num_samples));
        // sum += probabilities[i];
        // std::cout << probabilities[i] <<"\n";
      }
      std::vector<count_t> input(vocabulary_size);
      normalizer = (std::numeric_limits<count_t>::max() / vocabulary_size)
        / probabilities[0];
      for(size_t i = 0; i < vocabulary_size; i++) {
